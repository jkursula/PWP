...
def test_create_instances(db_handle):
    ...
    user2 = _get_user(username="user2", password="password")
    ...
    transaction = _get_transaction(price=3.50, dateTime=datetime.now(), sender=user1, receiver=user2, category=category1)
    ...
    assert db_transaction.category in db_categories
    assert db_transaction.sender.bankAccount in db_bankAccounts
